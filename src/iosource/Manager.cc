...
bool Manager::Poll(std::set<IOSource*>& ready, double timeout, IOSource* timeout_src)
	...
	int ret = epoll_wait(event_queue, events.data(), events.size(), timeout == 0 ? 0 : -1);
	if ( ret == -1 )
		{
		...
		{
		DBG_LOG(DBG_MAINLOOP, "epoll says %d things are ready", ret);
		for ( int i = 0; i < ret; i++ )
			{
			DBG_LOG(DBG_MAINLOOP, "%d is ready, status is %d", events[i].data.fd, events[i].events);
