...
void ProfileLogger::Log()
	...
	*/
	...
	// TODO: this also used to include a call to cq_memory_allocation from the CQ timer manager.
	// It needs to be replaced with something else to make the stats accurate again.
	file->Write(fmt("%.06f Timers: current=%d max=%d mem=%dK lag=%.2fs\n",
		network_time,
		timer_mgr->Size(), timer_mgr->PeakSize(),
		int(timer_mgr->Size() * padded_sizeof(ConnectionTimer)) / 1024,
